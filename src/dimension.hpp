...
class dimension {
  ...
  unsigned char rank; // how many dim are valid
...
public:
  ...
  explicit dimension(const SizeT *d, unsigned char nD = MAXRANK) {
    rank = nD;
  ...
  explicit dimension(const DLong *d, unsigned char nD = MAXRANK) {
    rank = nD;
    for (unsigned char i = 0; i < rank; ++i)
      dim[i] = d[i];
    stride[0] = 0; // not set
  ...
  explicit dimension(const SizeT d0) {
    assert(d0 != 0);
    //     if( d0 == 0) 
    //       {
    // 	rank = 0; return;
  ...
  void operator>>(const SizeT add) {
    unsigned char thisRank = rank++;

    if (rank > MAXRANK)
      throw GDLException("Only " + MAXRANK_STR + " dimensions allowed.");

    for (auto i = thisRank - 1; i >= 0; i--) {
      dim[i + 1] = dim[i];
    }
  ...
  SizeT EquivalentRank() const {
    unsigned char eqrank = rank;
    unsigned char i = 0;
    for (; i < eqrank && dim[i] <= 1; ++i);
    for (; eqrank > 1 && dim[eqrank - 1] <= 1; --eqrank);
    return eqrank - i;
  }
  // throw away unused ranks (i.e. ranks == 1)
  inline void Purge() {
